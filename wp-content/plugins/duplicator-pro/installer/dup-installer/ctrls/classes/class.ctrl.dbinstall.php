            ...
            case self::DBACTION_RENAME:
                ...
                if ($result       = DUPX_DB::mysqli_query($this->dbh, $sql)) {
                    ...
                    if (count($found_tables) > 0) {
                        $escapedDbName = mysqli_real_escape_string($this->dbh, $this->post['dbname']);
                        foreach ($found_tables as $table_name) {
                            $escapedOldTable = mysqli_real_escape_string($this->dbh, $table_name);
                            $ecapedNewTable  = mysqli_real_escape_string($this->dbh, substr($GLOBALS['DB_RENAME_PREFIX'].$table_name, 0, 64));
                            $sql             = "RENAME TABLE `".$escapedDbName."`.`".$escapedOldTable."` TO  `".$escapedDbName."`.`".$ecapedNewTable."`";
                            if (!$result          = DUPX_DB::mysqli_query($this->dbh, $sql)) {
