...
if ($element_id && $post_id) {
    ...
    $element = \DynamicContentForElementor\DCE_Helper::get_elementor_element_by_id($element_id);
    ...
    if ($template_id || $post_id) {
        ...
        if ($template_id) {
            ...
            if ($user_id) {
                //$set_author = ' author_id="' . $user_id . '"';
            }
            $pdf_shortcode = '[dce-elementor-template id="' . $template_id . '"' . $set_author . $set_post . ']';
            //echo $pdf_shortcode; die();
            //var_dump(get_queried_object()); die();
        ...
        $pdf_html = \DynamicContentForElementor\DCE_Helper::get_dynamic_value($pdf_html);
        //var_dump($pdf_html); die();

        if ($styles != 'unstyled') {
            // add CSS
            $css_id = $template_id ? $template_id : $post_id;
            $css = \DynamicContentForElementor\DCE_Helper::get_post_css($css_id, ($styles == 'all'));
            // from flex to table
        ...
        $pdf_html = \DynamicContentForElementor\DCE_Helper::template_unwrap($pdf_html);
        //$pdf_html = 'hello';
        //var_dump($pdf_html); die();
        ...
        switch($converter) {
            ...
            case 'tcpdf':
                ...
                // from div to table
                $pdf_html = \DynamicContentForElementor\DCE_Helper::tablefy($pdf_html);
