...
class DCE_Elements {
    ...
    public function dce_elements_init() {
        ...
        if (!is_admin()) {
            if (!DCE_Helper::is_edit_mode()) {
                // elements report
                add_action("elementor/frontend/widget/before_render", array($this, 'start_element'), 11, 2);
                $excluded_globals = json_decode(get_option(SL_PRODUCT_ID . '_excluded_globals'), true);
                if (empty($excluded_globals['DCE_Frontend_Navigator'])) {

                    self::$user_can_elementor = DCE_Helper::user_can_elementor();
    ...
    public static function get_template_ids() {
        ...
        $template_id = DCE_TemplateSystem::get_template_id(true);
        if ($template_id) {
            $templates['dce'] = $template_id;
        }

        // check with Elemento PRO Theme Builder
        $pro_template_id = DCE_Helper::get_theme_builder_template_id();
        if ($pro_template_id) {
            $templates['pro'] = $pro_template_id;
        }

        $post_template_id = DCE_TemplateSystem::get_post_template_id();
        if ($post_template_id) {
            ...
            if (is_string($element) || is_array($element) || $template_id) {
                if (!$template_id) {
                    if (is_string($element)) {
                        $template_id = DCE_Helper::get_template_id_by_html($element);
                    }
                    if (!$template_id && is_array($element)) {
                        //echo '<pre>';var_dump($element);echo '</pre>'; die();
                        //$template_id = $this->get_last_template_id();
                        $template_id = get_the_ID();
                        $template_id = DCE_Helper::get_post_id_by_element_data($element, $template_id);
                        //var_dump($template_id);
    ...
    public function add_elementor_navigator($admin_bar) {
        ...
        $admin_bar->add_menu(array(
            ...
            //'title' => '<i class="eicon-elementor" style="font-family: eicons;"></i> Navigator',
        ...
        /* wp_register_style(
          'dce-style-admin-bar', plugins_url('/assets/css/dce-admin-bar.css', DCE__FILE__), [], DCE_VERSION
          ); */
        DCE_Assets::dce_icon();
    ...
    public function print_elementor_submenu($template_ids) {
        if (!empty($template_ids)) {
            ...
            public function parse_elementor_data() {
                if (self::$elementor_data_current != 'ooo') {
                    ...
                    foreach (self::$elementor_data as $ekey => $edata) {
                        $kpos = explode(' > ', $ekey);
                        $tmp = DCE_Helper::set_array_value_by_keys($tmp, $kpos, $edata);
                    }
            ...
            public function print_frontend_navigator() {
                ...
                /* wp_register_style(
                  'dce-style-editor-navigator', plugins_url('/assets/css/dce-editor-navigator.css', DCE__FILE__), [], DCE_VERSION
                  );
                  wp_register_script(
                  'dce-script-editor-navigator', plugins_url('/assets/js/dce-editor-navigator.js', DCE__FILE__), [], DCE_VERSION
                  ); */
        ...
        //http://localhost/wp-content/plugins/elementor/assets/css/editor.css?ver=2.9.7
        ?>
                    </div>
                </div>
            ...
            if (is_array($elementor_data)) {
                ...
                foreach ($elementor_data as $ekey => $evalue) {
                    ...
                    <?php } ?>
                                ...
                                <div class="elementor-navigator__element__indicator" data-section="section_toggle" original-title="Toggle" <?php if (!self::$user_can_elementor || !isset(self::$elements_hidden[$id])) { ?>style="display: none;"<?php } ?>>
                    ...
                    <?php } ?>
                                        ...
                                        if ($type == 'template') {
                                            ...
                                            if ($document) {
                                                ...
                                                <?php if (self::$user_can_elementor) { ?>
                                                    ...
                                                    <dt><?php _e('Author', 'elementor'); ?></dt> <dd><a href="<?php echo \DynamicContentForElementor\DCE_Helper::get_user_link($document->get_post()->post_author); ?>" target="_blank"><?php echo get_the_author_meta('display_name', $document->get_post()->post_author); ?></a></dd>
                                                    ...
                                                    <?php } ?>
                                                        ...
                                                        if ($modified_author_id && $document->get_post()->post_author != $modified_author_id) {
                                                            ?>
                                                        <dt><?php _e('Modified by', 'elementor'); ?></dt>  <dd><a href="<?php echo \DynamicContentForElementor\DCE_Helper::get_user_link($modified_author_id); ?>" target="_blank"><?php echo get_the_author_meta('display_name', $modified_author_id); ?></a></dd>
                                                    <?php }
                                                } ?>
                        ...
                        <?php if (empty(self::$elements_hidden[$id])) { ?>
                                        ...
                                        $element_settings = DCE_Helper::get_elementor_element_settings_by_id($id, $template_id);
                                        foreach (self::$elements_hidden[$id]['triggers'] as $ckey => $cond) {
                                            ?>
                                                <dt><?php echo $cond; ?></dt> <dd><?php
                                                if (!empty($element_settings[$ckey])) {
                                                    echo DCE_Helper::to_string($element_settings[$ckey]);
                                                }
                                                ?></dd>
                                                <?php
                                            }
                    ...
                    if (!empty(self::$elements_settings[$id])) {
                            ...
                            if ($type != 'template') {
                                $element_settings = DCE_Helper::get_element_by_id($id, $template_id);
                            }
                        }
                    }
    ...
    public function get_element_title_by_id($id, $type, $name, $template_id = 0) {
        ...
        if ($type == 'section') {
            $settings = DCE_Helper::get_elementor_element_settings_by_id($id, $template_id);
            //var_dump($settings);
