...
use \DynamicContentForElementor\Includes\Settings\DCE_Settings_Manager;
...
class DCE_Settings {
    ...
    public function init() {
        ...
        if (is_admin()) {
            ...
            */
            ...
            $dce_apis = get_option(SL_PRODUCT_ID . '_apis');
            //var_dump($dce_apis); die();
            if (!empty($dce_apis['dce_api_gmaps']) && !empty($dce_apis['dce_api_gmaps_acf'])) {
                //var_dump($dce_options); die();
                if (DCE_Helper::is_plugin_active('advanced-custom-field-pro')) {
                    add_action('acf/init', function() {
                        $dce_apis = get_option(SL_PRODUCT_ID . '_apis');
                        acf_update_setting('google_api_key', $dce_apis['dce_api_gmaps']);
                    });
                } else if (DCE_Helper::is_plugin_active('acf')) {
    ...
    public function dce_setting_page($tplsys = false) {
        ...
        <div class="wrap">
            ...
            <div id="dce-settings-tabs-wrapper" class="nav-tab-wrapper">
                ...
                <a id="dce-settings-tab-globals" class="nav-tab<?php if (isset($_GET['tab']) && $_GET['tab'] == 'globals') { ?> nav-tab-active<?php } ?>" href="?page=dce_opt&tab=globals">
                    <span class="dashicons dashicons-menu"></span> <?php _e('Globals', 'dynamic-content-for-elementor'); ?>
                    <span class="dce-badge"><?php $globals = DCE_Settings_Manager::get_settings(); echo count($globals); ?></span>
                </a>
    ...
    public function show_globals_form() {
        ...
        <form action="" method="post">
            <?php
            $globals = DCE_Settings_Manager::get_settings();

            if (isset($_POST['save-dce-globals'])) {
                update_option(SL_PRODUCT_ID . '_active_globals', json_encode($_POST['dce-globals']));
                ...
                //$globals_tmp['dce_frontend_navigator_copy'] = 'DCE_Frontend_Navigator_Copy';
                ...
                update_option(SL_PRODUCT_ID . '_excluded_globals', json_encode($excluded_globals));
                DCE_Notice::dce_admin_notice__success(__('Your preferences have been saved. You will only see the active globals.', 'dynamic-content-for-elementor'));
            }
            $excluded_globals = json_decode(get_option(SL_PRODUCT_ID . '_excluded_globals'), true);
            //var_dump($excluded_globals); die();
            $i = 0;
            DCE_Settings_Manager::includes();
            ?>
            <table class="widefat dce-form-table dce-filter-table">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    foreach ($globals as $key => $className) {
                        $myWdgtClass = DCE_Settings_Manager::$namespace . $className;
                        if ($myWdgtClass::is_enabled()) {
                            //$myWdgtObj = new $myWdgtClass();
                            $pezzi = explode('_', $className, 3);
                            ?>
                            <tr>
                                <td> &nbsp; &nbsp;
                                    ...
                                    <label style="font-weight: bold;" for="dce-globals-<?php echo $className; ?>">
                                        <?php echo $myWdgtClass::$name; //echo end($pezzi);  ?>
                                    </label>
                                </td>
                            </tr>
                            <?php
                ...
                <tbody>
                    <tr>
                        <td>
                            ...
                            <input class="dce-apis dce-apis-gmaps" type="text" name="dce-apis[dce_api_gmaps]" id="dce-apis-gmaps" value="<?php echo (isset($dce_apis['dce_api_gmaps'])) ? $dce_apis['dce_api_gmaps'] : ''; ?>"><br>
                            <?php if (DCE_Helper::is_plugin_active('acf')) { ?>
                ...
                <div id="nav-menus-frame" class="wp-clearfix">
                    ...
                    <div id="menu-management-liquid">
                        <div id="menu-management">
                            <!--<div class="dce-template-panel dce-template-header">
                                <div class="dce-template-icon dce-template-before-icon">
                                    <div class="dce-template-icon-bar dce-template-before-icon-bar"></div>
                                </div>
                                <h4>Header</h4>
                                <?php
                                $dce_key = 'dyncontel_header';
                                echo $this->_dce_settings_select_template($dce_key, $templates);
                                ?>
                            </div>-->
                            <?php
                            $i = 0;
