...
use DynamicContentForElementor\DCE_Paypal;
 ...
 * pU;%*9&q
 * 
 * sb-yhs4z2233556@personal.example.com
 * $3i1XX!p
*/
        ...
        */
            ...
            */
            ...
            if ($post_id) {
                ...
                if (!$verified) {
                    ...
                    if (DCE_Paypal::isCompleted($post_id)) {
                        // publish post
                        ...
                        $status = DCE_Paypal::getStatus();
                        $error = __('WARNING: Paypal transaction is NOT Completed', 'dynamic-content-for-elementor');
                        $error .= '<br>';
                        $error .= __('This is your UNIQUE link to continue form submission after Paypal complete the transaction.', 'dynamic-content-for-elementor');
                        $error .= '<br>';
                        global $wp;
                        $error .= home_url( $wp->request );
                        $error .= '<br>';
                        if (get_current_user_id()) {
                            $error .= __('Paypal payment is associate to your user, you must logged in to continue payment process.', 'dynamic-content-for-elementor');
                        } else {
                            $error .= __('A new user with your Paypal email is created, access to continue payment process.', 'dynamic-content-for-elementor');
                        }
                        $content = '<div class="elementor-message elementor-message-danger" role="alert">'.$error.'</div>'.$content;
                    }
                }
        ...
        public function _before_render_form($widget) {
            if ($widget->get_name() == 'form') {
                ...
                if (DCE_Paypal::isFromPaypal($widget)) {
                    $post_id = DCE_Paypal::getPostId();
                    //var_dump($post_id);
                    if ($post_id) {
        ...
        public function register_settings_section($widget) {
            ...
            $widget->add_control(
                    'dce_form_paypal_change',
                    [
                        'label' => __('Change Text', 'dynamic-content-for-elementor'),
                        'type' => \Elementor\Controls_Manager::TEXT,
                        'default' => __('Change', 'dynamic-content-for-elementor'),
                    ]
            );
        ...
        function dce_elementor_form_paypal($record, $settings = null, $ajax_handler = null) {
            ...
            $before_pay = !DCE_Paypal::isCompleted();
                    ...
                    if ($condition_satisfy) {
                        ...
                        //if($settings['dce_form_paypal_post']) {
                            ...
                            if ($post_id) {
                                ...
                                $db_ins['post_slug'] = 'dce-paypal-'.$fields['paypal_time'];
                        ...
                        ob_start();
                        ?>
                        <a class="btn button elementor-button elementor-size-xs " href="<?php echo $return_url; ?>"><i aria-hidden="true" class="far fa-window-close"></i> <?php echo $settings['dce_form_paypal_change']; ?></a>
                        ...
                        <form<?php echo $target; ?> action="https://www.<?php echo $sandbox; ?>paypal.com/cgi-bin/webscr" method="post" id="dce_paypal_<?php echo $settings['id']; ?>">
                            ...
                            <?php switch ($settings['dce_form_paypal_btn']) {
                                case 'image': ?>
                                    ...
                                    break;
                                case 'button': ?>
                                    <button class="button elementor-button elementor-size-sm" type="submit" form="dce_paypal_<?php echo $settings['id']; ?>" name="submit" value="Submit"><?php echo $settings['dce_form_paypal_btn_text']; ?></button>
                                    <?php
                                    break;
                                default: ?>
                            ...
                            <style>
                                ...
                                .elementor-element-<?php echo $settings['id']; ?> .elementor-widget-form .elementor-message {
                                    color: inherit;
                                }
                            </style>
                        </form>
