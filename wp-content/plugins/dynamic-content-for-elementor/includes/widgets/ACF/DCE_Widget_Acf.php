...
class DCE_Widget_Acf extends DCE_Widget_Prototype {
    ...
    protected function _register_controls() {
        ...
        $this->add_control(
                'acf_type', [
            'label' => __('ACF type of render', 'dynamic-content-for-elementor'),
            'type' => Controls_Manager::SELECT,
            'options' => [
    ...
    protected function render() {
        ...
        $fieldSettings = DCE_Helper::get_acf_field_settings($idFields);
        ...
        // ------------------------------------------
        $acf_field_value = DCE_Helper::get_acf_field_value($idFields, $id_page);
        ...
        if ($typeField == 'text' ||
            ...
            if ($typeField == 'list') {
                ...
                switch ($fieldSettings['type']) {
                    case 'radio':
                        if (is_array($acf_field_value)) {
                            ...
                            } else if ($settings['list_array_return'] == 'label') {
                                $acfResult .= '<span class="dce-acf-list dce-acf-list-label">' . $acf_field_value['label'] . '</span>';
                            } else if ($settings['list_array_return'] == 'value') {
                    ...
                    default:
                        if (!empty($acf_field_value)) {
                            ...
                            foreach ($acf_field_value as $akey => $field) {
                                $label = DCE_Helper::to_string($field);
                                if ($akey > 0)
                                    $acfResult .= '<span class="dce-acf-list-separator">' . $settings['list_separator'] . '</span>';
                ...
                $acf_field_value = DCE_Helper::to_string($acf_field_value);
            ...
            if (\Elementor\Plugin::$instance->editor->is_edit_mode() && get_post_type($elibpost) == 'elementor_library' && (isset($field_settings['type']) && $field_settings['type'] != 'repeater'))
                ...
                } else if (!$acfResult && $typeField == 'list') {
                    $acfResult = 'ListField';
            ...
            $acfResult = $acf_field_value; //get_field( $idFields , $id_page);
            // Default ;-)
            ...
            $videoField = $acf_field_value; //get_field( $idFields , $id_page, false);
            // Default ;-)
            if ($type_page == 'elementor_library')
                if ($videoField == '')
                ...
                if ($this->has_image_overlay()) {
                    ...
                    if (!$settings['lightbox']) {
                        if ($settings['image_overlay_type'] == 'custom') {
                            ...
                            $immagine_acf_overlay = DCE_Helper::get_acf_field_value($settings['image_overlay_acf'], $id_page);
        ...
        switch ($settings['link_to']) {
            ...
            case 'acf_url' :
                ...
                if (!empty($settings['acf_field_url'])) {
                    $link = esc_url(DCE_Helper::get_acf_field_value($settings['acf_field_url'], $id_page));
