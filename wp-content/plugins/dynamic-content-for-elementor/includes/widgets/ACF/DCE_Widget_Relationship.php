    ...
    protected function render() {
        ...
        if ($settings['acf_relation_field']) {
            ...
            if ($settings['acf_relation_invert']) {
                $rel_post = DCE_Helper::get_acf_field_value_relationship_invert($settings['acf_relation_field']);
            } else {
                $rel_post = get_post_meta(get_the_ID(), $settings['acf_relation_field'], true);
                if (empty($rel_post)) {
                    $rel_post = DCE_Helper::get_acf_field_value($settings['acf_relation_field']);
                }
            }
            //var_dump($rel_post);
            if ($rel_post) {
                ...
                if (is_array($rel_post) && !empty($rel_post)) {
                    ...
                    // remove duplicates
                    $rel_post = array_unique($rel_post);
                    acf_remove_loop('active'); // fix for ACF Repeater
                    ...
                    if (count($rel_post) >= 1 && $settings['acf_relation_format']) {
                        ...
                        if (in_array($settings['acf_relation_format'], array('tab', 'accordion', 'select'))) {
                            foreach ($rel_post as $arel) {
                                $arel = DCE_Helper::get_rev_ID($arel);
                                $post = get_post($arel);
                                if (!$settings['acf_relation_draft'] && $post->post_status == 'draft') {
                                    continue;
                                }
                                //var_dump($post->ID);
                                //setup_postdata( $post );
                                $labels[$post->ID] = DCE_Helper::get_dynamic_value($settings['acf_relation_label']);
                            }
                        }
                        ...
                        foreach ($rel_post as $rkey => $arel) {
                            ...
                            //var_dump($post);
                            if (count($rel_post) && $settings['acf_relation_format']) {
                                switch ($settings['acf_relation_format']) {
                                    case 'ul':
                                    case 'ol':
                            ...
                            if (!$settings['acf_relation_format'] && $rkey) {
