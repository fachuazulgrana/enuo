...
class DCE_Widget_Repeater extends DCE_Widget_Prototype {
    ...
    protected function _register_controls() {
        ...
        foreach ($repeaters as $arepeater => $arepeater_title) {
                ...
                $link_fields = DCE_Helper::get_acf_fields(array('url', 'image', 'file', 'link', 'post_object', 'page_link', 'taxonomy', 'user'));
                $repeater_fields->add_control(
    ...
    protected function render() {
        ...
        if ($settings['dce_acf_repeater']) {
            ...
            $locations = DCE_Helper::get_acf_field_locations($settings['dce_acf_repeater']);
            //var_dump($locations);
            ...
            if (have_rows($settings['dce_acf_repeater'], $id_page)) {
                ...
                //var_dump($settings['selected_icon']);
                        ...
                        switch ($settings['dce_acf_repeater_format']) {
                            case 'list':
                                ...
                                if ($settings['dce_acf_repeater_list'] == 'ul') {
                                    echo '<span class="elementor-icon">' . DCE_Helper::get_icon($settings['selected_icon_ul']) . '</span>';
                                }
                                break;
                            case 'grid':
                            ...
                            case 'accordion':
                                ...
                                echo '<div class="dce-acf-repeater-item elementor-accordion-item"><' . $settings['dce_acf_repeater_accordion_heading_size'] . ' class="elementor-tab-title' . $element_active . '" data-tab="' . $this->get_id() . '-' . $row_id . '" role="tab" aria-controls="elementor-tab-content-' . $this->get_id() . '-' . $row_id . '"><span class="elementor-accordion-icon elementor-accordion-icon-' . esc_attr($settings['icon_align']) . '" aria-hidden="true"><span class="elementor-accordion-icon-closed">' . DCE_Helper::get_icon($settings['selected_icon']) . '</span><span class="elementor-accordion-icon-opened">' . DCE_Helper::get_icon($settings['selected_active_icon']) . '</span></span><a class="elementor-accordion-title" href="#" onclick="return false;"> ';
                                echo \DynamicContentForElementor\DCE_Tokens::replace_var_tokens($settings['dce_acf_repeater_accordion_heading'], 'ROW', $row_fields);
                                        ...
                                        if ($acfitem['dce_acf_repeater_field_show']) {
                                            ...
                                            if (!empty($value)) {
                                                ...
                                                switch ($field_type) {
                                                    ...
                                                    case 'textarea':
                                                    default:
                                                        $value = DCE_Helper::to_string($value, true);
                                                }
                                            }
                                            if (!empty($row_fields[$acfitem['dce_acf_repeater_acfield_link']]) && !empty($acfitem['dce_acf_repeater_enable_link'])) {
                                                ...
                                                $link = DCE_Helper::get_permalink($link, '#');                                                
