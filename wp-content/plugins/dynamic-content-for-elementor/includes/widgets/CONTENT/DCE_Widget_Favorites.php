...
class DCE_Widget_Favorites extends DCE_Widget_Prototype {
    ...
    protected function _register_controls() {
        ...
        $this->add_control(
            ...
            'label' => __('Is cache enabled', 'dynamic-content-for-elementor'),
    ...
    protected function render() {
        ...
        if (!empty($settings['dce_favorite_list'])) {
                ...
                <?php if (count($settings['dce_favorite_list']) > 1) { ?>
                    ...
                    if ($is_favorite && !$settings['dce_favorite_remove']) {
                        ...
                        if (!$user_id && $settings['dce_favorite_scope'] == 'user' && !$settings['dce_favorite_visitor_hide']) {
                            ...
                            /*
                              $btn_url = remove_query_arg('eid', $btn_url);
                              $btn_url = remove_query_arg('dce_list', $btn_url);
                              $btn_url = remove_query_arg('dce_post_id', $btn_url);
                             */
                    ...
                    if ($settings['hover_animation']) {
                        $this->add_render_attribute('button', 'class', 'elementor-animation-' . $settings['hover_animation']);
                        //echo '<div class="elementor-animation-' . $settings['hover_animation'] . '">';
                    }
                    ?>
            ...
            <?php if (($settings['dce_favorite_ajax'] || $act_add || \Elementor\Plugin::$instance->editor->is_edit_mode()) && $settings['dce_favorite_msg_add_enable']) { ?>
                <div class="elementor-message elementor-message-success dce-notice dce-notice-favorite dce-notice-favorite-add elementor-alert elementor-alert-success<?php echo $modal_class; ?>"<?php if ($settings['dce_favorite_ajax'] && !\Elementor\Plugin::$instance->editor->is_edit_mode()) { ?> style="display:none;"<?php } ?>>
            ...
            if (($settings['dce_favorite_ajax'] || $act_remove || \Elementor\Plugin::$instance->editor->is_edit_mode()) && $settings['dce_favorite_msg_remove_enable']) {
                ?>
                <div class="elementor-message elementor-message-danger dce-notice dce-notice-favorite dce-notice-favorite-remove elementor-alert elementor-alert-warning<?php echo $modal_class; ?>"<?php if ($settings['dce_favorite_ajax'] && !\Elementor\Plugin::$instance->editor->is_edit_mode()) { ?> style="display:none;"<?php } ?>>
    ...
    public function update_list($element_ID, $post_ID, $list_key = '') {
        ...
        if (!empty($_GET['eid']) && $_GET['eid'] == $element_ID) {
            if (!empty($_GET['dce_list'])) {
                if (!empty($_GET['dce_post_id']) && $_GET['dce_post_id'] == $post_ID) {
                    $list_key = (!empty($_GET['dce_list'])) ? $_GET['dce_list'] : $list_key;
                    ...
                    /* if (!$settings['dce_favorite_ajax']) {
                      // redirect to update other dynamic widgets related to him
                      $page_url = remove_query_arg('eid');
                      $page_url = remove_query_arg('dce_list', $page_url);
                      $page_url = remove_query_arg('dce_post_id', $page_url);
    ...
    public static function get_user_counter($list_key = '') {
        global $wpdb;
        $sql = "SELECT COUNT(user_id) as ucount FROM " . $wpdb->prefix . "usermeta um WHERE meta_key LIKE '" . $list_key . "' AND meta_value LIKE '%i:" . get_the_ID() . ";%'";
        //var_dump($sql);
    ...
    public static function get_cookie_counter($list_key = '', $post_ID) {
        $cookies = get_option('dce_favorite_cookies', array());
        if (isset($cookies[$list_key][$post_ID])) {
