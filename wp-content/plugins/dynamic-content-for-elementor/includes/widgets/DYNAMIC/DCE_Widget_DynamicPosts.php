...
class DCE_Widget_DynamicPosts extends DCE_Widget_Prototype {
    ...
    public function get_script_depends() {
        return ['imagesloaded', 'jquery-slick', 'swiper', 'isotope', 'infinitescroll', 'wow', 'dce-ajaxmodal', 'dce-acf_posts'];
    }
    ...
    protected function _register_controls() {
        ...
        /*$this->add_control(
            ...
            'options' => DCE_Helper::get_acf_field_relational_post(), //get_acf_fields('relationship'),
            'default' => '0',
            'condition' => [
    ...
    protected function render() {
        ...
        if ($settings['terms_from_acf'] && $settings['acf_taxonomy']) {
            $acfterm = DCE_Helper::get_post_meta($id_page, $settings['acf_taxonomy']);
            $terms_query = array();
            if (!empty($acfterm)) {
        ...
        if ($settings['query_type'] == 'specific_posts') {
            ...
            if ($settings['acf_relationship_invert']) {
                $relations_ids = DCE_Helper::get_acf_field_value_relationship_invert($settings['acf_relationship'], $id_page);
            } else {
            ...
            if ($settings['filters_acf']) {
                ...
                if ($p_query->have_posts()) :
                    while ($p_query->have_posts()) : $p_query->the_post();
                        $id_page = get_the_ID();
                        $acffield = DCE_Helper::get_post_meta($id_page, $settings['filters_acf']);
                ...
                // ***************************************************
                //$term_list_filters = array_unique( $term_list_filters );
            ...
            if (!empty($term_list_filters)) {
                ...
                foreach ($term_list_filters as $fkey => $filter) {
                    ...
                    if ((is_object($filter) && get_class($filter) == 'WP_Term') || $settings['filters_acf']) {
                        ...
                        $term_url = (is_object($filter) && get_class($filter) == 'WP_Term') ? DCE_Helper::get_term_link($filter->term_id) : '#';
                ...
                while ($p_query->have_posts()) : $p_query->the_post();
                    ...
                    if ($settings['filters_enable']) {
                        ...
                        if ($settings['filters_acf'] != '0') {
                            ...
                            $slug = DCE_Helper::get_post_meta($id_page, $acf_field);
                    ...
                    <div data-dce-post-id="<?php echo $id_page; ?>" class="dce-post-item dce-post-item-<?php echo $id_page; ?> dce-elementor-<?php echo $id_page; ?> column-<?php echo $settings['columns_grid'] . $classItem; ?>  <?php echo $filters_string_class . $stringSameHeightItem . $wow_string . $hoverEffectsClass; ?>"<?php echo $wow_delay; ?>>
                        ...
                        <div class="dce-wrapper <?php echo $animation_class; ?>">
                            ...
                            if ($settings['templatemode_enable'] == '' && $settings['native_templatemode_enable'] == '') {
                                ...
                                <?php if ($hoverEffects_enable) echo '</div>'; ?>
                                <?php
                                // deprecated: last version
    ...
    private function generate_acfitems($settings, $id_page = null) {
        ...
        //var_dump($id_page);
        //$settings_acf_items = DCE_Helper::get_elementor_element_settings_by_id($this->get_id()); //$this->get_settings('acf_items');
        //echo '<pre>';var_dump($settings_acf_items); echo '</pre>';
        ...
        if (!empty($ACFitems)) {
            foreach ($ACFitems as $acfkey => $acfitem) :
                ...
                switch ($acfitem['link_to']) {
                    case 'custom' :
                        if (!empty($acfitem['link']['url'])) {
                            ...
                            if ($acf_name) {
                                //$link = get_field($acf_name);
                                $link = DCE_Helper::get_post_meta($id_page, $acf_name);
                            }
                        }
                        break;
                    case 'post' :
                ...
                //var_dump($settings_acf_items[$acfkey]['link']);
                if ($link) {
                ...
                if ($acf_i == 'title') {
                    ...
                    $acf_i_val = DCE_Helper::get_post_meta( $id_page, $acf_i );
                    //    
                    if ($acf_type_i == 'image') {
                        //echo $acf_i_val;
                        ...
                        $acf_i_val = DCE_Helper::to_string($acf_i_val);
                        //var_dump($acf_i_val);
    ...
    protected function generate_image($settings, $clss = null) {
        ...
        <div class="dce-acfposts_image<?php echo $clss; ?>">
            <?php if (!$settings['use_bgimage']) echo '<div class="dce-acfposts_imagewrap">' ?>
            ...
            <?php if (!$settings['use_bgimage']) echo '</div>' ?>
        </div>
