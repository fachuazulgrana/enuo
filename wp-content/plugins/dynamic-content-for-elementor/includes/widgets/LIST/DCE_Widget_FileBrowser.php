...
class DCE_Widget_FileBrowser extends DCE_Widget_Prototype {
                ...
                if ($settings['path_selection'] == 'csv') {
                    if ($settings['folder_csv']) {
                                                ...
                                                if (array_key_exists($filename, $files)) {
                                                    ...
                                                    if (!empty($chunk)) {
                                                        foreach($chunk as $kkey => $cnk) {
                                                            $tmp[] = $cnk;
                                                            $arr_key = '["'.implode('"]["', $tmp).'"]';
                                                            $tmp_value = DCE_Helper::get_array_value_by_keys($dirs, $tmp);
                                                            if (!$tmp_value) {
                                                                $dirs = DCE_Helper::set_array_value_by_keys($dirs, $tmp, array());
                                                            }
                                                            ...
                                                            if ($kkey == count($chunk)-1) {
                                                                ...
                                                                $tmp_add = $tmp;
                                                                $tmp_add[] = $dkey;
                                                                $dirs = DCE_Helper::set_array_value_by_keys($dirs, $tmp_add, array($filename_title => $filename));
                        ...
                        if ($settings['folder_csv_filter']) {
                            $folder_csv = explode($settings['folder_csv_separator'], $settings['folder_csv_filter']);
                            $tmp_value = DCE_Helper::get_array_value_by_keys($dirs, $folder_csv);
                            if ($tmp_value) { 
    ...
    public function dirToHtml($dir, $hidden = false, $files = array(), $dirs = array()) {
        ...
        if (empty($cdir)) {
            ...
            //$cdir = DCE_Helper::scandir_by_date($dir); // sort by date
        }
                ...
                if (is_array($value) || is_dir($fulldir)) {
                    ...
                    if (!DCE_Helper::is_empty_dir($fulldir) || $settings['empty']) {
                        ...
                        if (\Elementor\Plugin::$instance->editor->is_edit_mode() || !$hide) {
                            ...
                            $direct_link = DCE_Helper::path_to_url($fulldir);
                            if ($settings['path_selection'] == 'uploads' || $settings['path_selection'] == 'custom') {
                                if (!empty($settings['private_access'])) {
