...
class DCE_QueryControl extends Module {
    ...
    protected function get_posts($data) {
        ...
        if ($object_type == 'type') {
            $list = \DynamicContentForElementor\DCE_Helper::get_post_types();
            if (!empty($list)) {
    ...
    protected function get_acf($data) {
        ...
        $acfs = \DynamicContentForElementor\DCE_Helper::get_acf_fields($types);
        if (!empty($acfs)) {
    ...
    protected function get_users($data) {
        ...
        if ($object_type == 'role') {
            $list = \DynamicContentForElementor\DCE_Helper::get_roles();
    ...
    protected function get_value_titles_for_acf($request) {
        ...
        $results = [];
        foreach ($ids as $aid) {
            $acf = \DynamicContentForElementor\DCE_Helper::get_acf_field_post($aid);
            if ($acf) {
    ...
    protected function get_value_titles_for_posts($request) {
        ...
        if ($is_ctp) {
            $post_types = \DynamicContentForElementor\DCE_Helper::get_post_types();
            if (!empty($ids)) {                
                foreach ($ids as $aid) {
    ...
    protected function get_value_titles_for_users($request) {
        ...
        if ($is_role) {
            $roles = \DynamicContentForElementor\DCE_Helper::get_roles();
