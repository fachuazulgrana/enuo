		...
		<div id="<?php echo $id; ?>_wrap" class="editfield bps-editfield bps-<?php echo $display; ?>">
			...
			<?php
				...
				if ( 'datebox' == $filter_escaped_item->type ) {
					...
					echo "<option  value=''>" . esc_html__( 'from', 'bp-profile-search' ) . '</option><option disabled>──────────</option>';
					for ( $k = $age_range_start; $age_range_end > $k; $k++ ) {
						$selected = ! empty( $filter_escaped_item->min ) && ( $k === intval( $filter_escaped_item->min ) ) ? "selected='selected'" : '';
					...
					echo "<option  value=''>" . esc_html__( 'to', 'bp-profile-search' ) . '</option><option disabled>──────────</option>';
					for ( $k = $age_range_start; $age_range_end > $k; $k++ ) {
						$selected = ! empty( $filter_escaped_item->max ) &&  ( $k === intval( $filter_escaped_item->max ) ) ? "selected='selected'" : '';
						echo "<option $selected value='$k'>$k</option>";
					}
					echo '</select></div></div>';
					echo '</div>';
						...
						<select id="<?php echo $id; ?>" name="<?php echo $name ?>">
							...
							<?php $no_selection = apply_filters( 'bps_field_selectbox_no_selection', $filter_escaped_item->label, $filter_escaped_item );
								...
								foreach ( $filter_escaped_item->options as $key => $label ) {
									if ( ! empty( $key ) ) {
										$selected = ! empty( $filter_escaped_item->values ) && in_array( $key, $filter_escaped_item->values ) ? "selected='selected'" : '';
