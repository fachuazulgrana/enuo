	...
	protected function add_text_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'    => $outputs,
				'css_vars'  => array(
					array( '--grimlock-text-font-family', '$', 'font-family' ),
					array( '--grimlock-text-font-weight', '$', 'font-weight' ),
					array( '--grimlock-text-font-size', '$', 'font-size' ),
					array( '--grimlock-text-line-height', '$', 'line-height' ),
					array( '--grimlock-text-letter-spacing', '$', 'letter-spacing' ),
					array( '--grimlock-text-text-transform', '$', 'text-transform' ),
				),
	...
	protected function add_heading_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'      => $outputs,
				'css_vars'    => array(
					array( '--grimlock-heading-font-family', '$', 'font-family' ),
					array( '--grimlock-heading-letter-spacing', '$', 'letter-spacing' ),
					array( '--grimlock-heading-text-transform', '$', 'text-transform' ),
				),
				'js_vars'     => $this->to_js_vars( $outputs ),
			) );
	...
	protected function add_heading1_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'   => $outputs,
				'css_vars' => array(
					array( '--grimlock-heading1-font-size', '$', 'font-size' ),
					array( '--grimlock-heading1-line-height', '$', 'line-height' ),
				),
			) );
	...
	protected function add_heading2_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'   => $outputs,
				'css_vars' => array(
					array( '--grimlock-heading2-font-size', '$', 'font-size' ),
					array( '--grimlock-heading2-line-height', '$', 'line-height' ),
				),
			) );
	...
	protected function add_heading3_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'   => $outputs,
				'css_vars' => array(
					array( '--grimlock-heading3-font-size', '$', 'font-size' ),
					array( '--grimlock-heading3-line-height', '$', 'line-height' ),
				),
			) );
	...
	protected function add_heading4_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'   => $outputs,
				'css_vars'  => array(
					array( '--grimlock-heading4-font-size', '$', 'font-size' ),
					array( '--grimlock-heading4-line-height', '$', 'line-height' ),
				),
			) );
	...
	protected function add_heading5_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'   => $outputs,
				'css_vars'  => array(
					array( '--grimlock-heading5-font-size', '$', 'font-size' ),
					array( '--grimlock-heading5-line-height', '$', 'line-height' ),
				),
			) );
	...
	protected function add_heading6_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'   => $outputs,
				'css_vars'  => array(
					array( '--grimlock-button-font-size', '$', 'font-size' ),
					array( '--grimlock-button-line-height', '$', 'line-height' ),
				),
			) );
	...
	protected function add_display_heading_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'      => $outputs,
				'css_vars'    => array(
					array( '--grimlock-display-heading-font-family', '$', 'font-family' ),
					array( '--grimlock-display-heading-letter-spacing', '$', 'letter-spacing' ),
					array( '--grimlock-display-heading-text-transform', '$', 'text-transform' ),
				),
				'js_vars'     => $this->to_js_vars( $outputs ),
			) );
	...
	protected function add_display_heading1_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'      => $outputs,
				'css_vars'    => array(
					array( '--grimlock-display-heading1-font-size', '$', 'font-size' ),
					array( '--grimlock-display-heading1-line-height', '$', 'line-height' ),
				),
			) );
	...
	protected function add_display_heading2_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'   => $outputs,
				'css_vars'  => array(
					array( '--grimlock-display-heading2-font-size', '$', 'font-size' ),
					array( '--grimlock-display-heading2-line-height', '$', 'line-height' ),
				),
			) );
	...
	protected function add_display_heading3_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'   => $outputs,
				'css_vars'  => array(
					array( '--grimlock-display-heading3-font-size', '$', 'font-size' ),
					array( '--grimlock-display-heading3-line-height', '$', 'line-height' ),
				),
			) );
	...
	protected function add_display_heading4_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'   => $outputs,
				'css_vars'  => array(
					array( '--grimlock-display-heading4-font-size', '$', 'font-size' ),
					array( '--grimlock-display-heading4-line-height', '$', 'line-height' ),
				),
			) );
	...
	protected function add_subheading_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'      => $outputs,
				'css_vars'    => array(
					array( '--grimlock-subheading-font-family', '$', 'font-family' ),
					array( '--grimlock-subheading-font-weight', '$', 'font-weight' ),
					array( '--grimlock-subheading-font-size', '$', 'font-size' ),
					array( '--grimlock-subheading-line-height', '$', 'line-height' ),
					array( '--grimlock-subheading-letter-spacing', '$', 'letter-spacing' ),
					array( '--grimlock-subheading-text-transform', '$', 'text-transform' ),
				),
				'js_vars'     => $this->to_js_vars( $outputs ),
			) );
	...
	protected function add_blockquote_font_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			...
			$args = wp_parse_args( $args, array(
				...
				'output'    => $outputs,
				'css_vars'  => array(
					array( '--grimlock-blockquote-font-family', '$', 'font-family' ),
					array( '--grimlock-blockquote-font-weight', '$', 'font-weight' ),
					array( '--grimlock-blockquote-font-size', '$', 'font-size' ),
					array( '--grimlock-blockquote-line-height', '$', 'line-height' ),
					array( '--grimlock-blockquote-letter-spacing', '$', 'letter-spacing' ),
					array( '--grimlock-blockquote-text-transform', '$', 'text-transform' ),
				),
				'js_vars'   => $this->to_js_vars( $outputs ),
			) );
