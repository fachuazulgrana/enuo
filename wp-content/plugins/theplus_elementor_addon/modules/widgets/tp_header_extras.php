						...
						foreach ( $settings['sequence_icons'] as $index => $item ) :
							...
							if((!empty($settings["display_language_switcher"]) && $settings["display_language_switcher"]=='yes') && $select_icon_list=='wpml_lang'){
								if(!empty($settings["select_trans"]) && $settings["select_trans"]=='p_wpml'){
									$wpml_style = $settings['wpml_style'];
									$wpml_style_layout = $settings['wpml_style_layout'];
									...
									if( !empty( $wpml ) ) { 									
											$meta_content .= '<li class="tp-wpml-wrapper elementor-repeater-item-' . $item['_id'] . ' '.esc_attr($responsive_class_attr).'">';
												
												if(!empty($settings['wpml_style']) && $settings['wpml_style']=='wpml_style_1'){
													$wpml_layout='';
													if(!empty($settings['wpml_style_layout']) && $settings['wpml_style_layout']=='tp-wpml-layout-v'){
														$wpml_layout = 'tp-wpml-layout-v';
													}else if(!empty($settings['wpml_style_layout']) && $settings['wpml_style_layout']=='tp-wpml-layout-h'){
														$wpml_layout = 'tp-wpml-layout-h';
													}
													
													$meta_content .= '<ul class="tp-wpml-menu '.$wpml_layout.'">';
													foreach( $wpml as $language ){
														$meta_content .=  '<li class="tp-wpml-menu-item">';
															
															$meta_content .=  ( $language['active'] ) ? '<a href="' . $language['url'] . '" class="tp-wpml-item tp-wpml-item__active">' : '<a href="' . $language['url'] . '" class="tp-wpml-item">';
															
																if($before_language_text){
																	$meta_content .= '<span class="tp-wpml-before-lt">' .$before_language_text.'</span>';	
																}
																
																if($display_country_flag){
																	$meta_content .= '<span class="tp-wpml-country-flag"><img src="' . $language['country_flag_url'] . '" alt="'.$language['language_code'].'" /></span>';
																}
																
																if($display_native_name){
																	$meta_content .= '<span class="tp-wpml-native-name">'.$language['native_name'].'</span>';
																}
																
																if($display_translated_name){
																	$meta_content .= '<span class="tp-wpml-translated-name">'.$language['translated_name'].'</span>';	
																}
																if($display_language_code){
																	$meta_content .=  '<span class="tp-wpml-language-code">'.$language['language_code'].'</span>';
																}
																if($after_language_text){
																	$meta_content .=  '<span class="tp-wpml-after-lt">'
																	.$after_language_text.'</span>';
																}
													...
													if($before_language_text){
														$meta_content .=  '<span class="tp-wpml-before-lt">'
																	.$before_language_text.'</span>';
													}
									...
									$before_language_text = !empty($settings['before_language_text']) ? $settings['before_language_text'] : '';
									$meta_content .= '<li class="tp-translatepress-wrapper elementor-repeater-item-' . $item['_id'] . ' '.esc_attr($responsive_class_attr).'">';
										$after_language_text = !empty($settings['after_language_text']) ? $settings['after_language_text'] : '';
										if($before_language_text){
											$meta_content .=  '<span class="tp-translatepress-before-lt">'.$before_language_text.'</span>';
										}
										$meta_content .= do_shortcode( $this->translatepress_get_shortcode() );									
										if($after_language_text){
											$meta_content .=  '<span class="tp-translatepress-after-lt">'.$after_language_text.'</span>';
										}
									$meta_content .='</li>';
								}								
							}
