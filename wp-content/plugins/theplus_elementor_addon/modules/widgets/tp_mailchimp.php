				...
				$output .='<form action="'.site_url().'/wp-admin/admin-ajax.php" id="'.esc_attr($id).'" class="theplus-mailchimp-form '.$content_align.' '.$content_align_tablet.' '.$content_align_mobile.'" data-thank-you="'.esc_attr($redirect_thankyou).'">';
					$output .='<div class="plus-newsletter-input-wrapper">';
						...
						if((!empty($settings["mc_cst_group"]) && $settings["mc_cst_group"]=='yes') && !empty($settings["mc_cst_group_value"])){
							$output .='<input type="hidden" name="mc_cst_group_value" value="'.$settings["mc_cst_group_value"].'" />';
						}
						if((!empty($settings["mc_cst_tag"]) && $settings["mc_cst_tag"]=='yes') && !empty($settings["mc_cst_tags_value"])){
							$output .='<input type="hidden" name="mc_cst_tags_value" value="'.$settings["mc_cst_tags_value"].'" />';
						}
